...
- Processed with table-aware extraction (pdfplumber)
...
| Table handling | pdfplumber structured | Poor | Poor |
...
| PDF processing | pdfplumber | Free | Replaces Textract ($600 one-time cost) |
| Embeddings | Amazon Titan Text Embeddings V2 | Free tier 3mo | Native Bedrock |
| Vector store | Pinecone free tier | Free | Replaces OpenSearch Serverless ($100/mo) |
...
pdfplumber over Textract:
Textract would cost $600 one-time for 500 filings at 80 pages each.
pdfplumber is free and handles SEC filing tables at 75-80% quality.
Good enough for the corpus. Use it.
...
│   ├── pdf_processor.py               # pdfplumber table + text extraction
...
Step 2: pdf_processor.py processes each PDF with pdfplumber
...
- [ ] pdf_processor.py: pdfplumber extraction for text + tables
...
Next task: Initialize repo + deploy storage CDK stack
Blockers: none
